...
### 修复
- **工作区出现两个重名「工单」标签**：「业主查询 → 工单查看」打开的是
  `/service-orders?buyer_id=N`，与菜单「工单」的 `/service-orders` 是两个 URL，
  工作区按完整 URL 去重所以两个标签并存；而页面 `<title>` 恒为「工单」，两个标签
  无法区分。现带过滤上下文时标题变为「工单 · <站点名>」（`?q=` 时为
  「工单 · 关键词 <词>」），页面工具栏副标题与底部状态栏也显示对应站点/关键词，
  让过滤状态在页面上可见。
