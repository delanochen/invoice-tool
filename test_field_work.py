...
class FieldWorkTest(unittest.TestCase):
    ...
    def test_nameplate_recognition_tolerates_ocr_label_damage_and_unlabeled_long_number(self):
        damaged = type('Result', (), {'returncode':0, 'stdout':b'Machlne Nurnber\n1023231239551\n'})()
        with patch.object(field_work.subprocess, 'run', return_value=damaged) as run:
            response = self.http.post('/api/field/recognize-equipment',
                                      data={'photo':(BytesIO(self.photo),'plate.jpg')},
                                      headers={'X-Field-Token':self.csrf})
        self.assertEqual(response.status_code, 200, response.text)
    ...
    def test_field_shell_uses_selected_language_and_caches_translations(self):
        ...
        self.assertIn('/static/field-work.js?v=', response.text)
        self.assertIn('/static/field-work.css?v=', response.text)
        self.assertIn('id="openCamera"', response.text)
        self.assertNotIn('id="confirmDevice"', response.text)
        self.assertNotIn('id="photoPreview"', response.text)
        ...
        self.assertIn('cache.put(url.pathname, response.clone())', worker)
    ...
    def test_system_photo_picker_uses_a_direct_native_input_click(self):
        ...
        self.assertNotIn('id="scanNameplate"', page)
        ...
        self.assertIn('.camera-active .camera-actions{position:fixed', field_css)
        ...
        self.assertIn("batch?.type !== 'equipment'", script)
        self.assertIn('clearTimeout(ocrStartTimer)', script)
