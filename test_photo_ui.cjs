...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe',headless:true});try{
 ...
 await page.waitForSelector('#ledgerPhotoDialog[open]');await page.locator('[data-image-preview-in]').click();assert.equal(await page.locator('#ledgerPhotoImage').evaluate(el=>parseFloat(el.style.width)/el.naturalWidth),1.25);
 await page.locator('[data-image-preview-out]').click();assert.equal(await page.locator('#ledgerPhotoImage').evaluate(el=>parseFloat(el.style.width)/el.naturalWidth),1);
